...
namespace Com.Bateeq.Service.Pos.Test.Controller.ReportControllerTests
{
    ...
    {
		protected SalesReturnReportController GetController(Mock<IIdentityService> identityService, Mock<IValidateService> validateService, Mock<ISalesDocReturnService> service)
		{
			...
			SalesReturnReportController controller = new SalesReturnReportController(identityService.Object, validateService.Object, service.Object);
		...
		public void Get_Return_Data()
		{
			...
			Mock<ISalesDocReturnService> service = new Mock<ISalesDocReturnService>();
			service.Setup(x => x.SalesReturnReport(It.IsAny<string>(), It.IsAny<DateTimeOffset>(), It.IsAny<DateTimeOffset>(), It.IsAny<string>())).Returns(new List<SalesDoc>() { });

			serviceProvider.Setup(s => s.GetService(typeof(ISalesDocReturnService))).Returns(service);
			//serviceProvider.Setup(s => s.GetService(typeof(PosDbContext))).Returns(dbContext);

			//SalesDoc testData = GetTestData(dbContext);

			//Act
			IActionResult response = GetController(identityService, validateService, service).Read(It.IsAny<string>(), It.IsAny<DateTimeOffset>(), It.IsAny<DateTimeOffset>(), It.IsAny<string>());
